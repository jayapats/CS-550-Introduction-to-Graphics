...
for (angle = 0.0f; angle <= 360 ; angle += 5)
{
	// x and y position of the next vertex
	dx = 1.0f * sin(angle);
	dy = 1.0f * cos(angle);
	...
	// Incrementing the flag to change color 
	flag++;

	// next vertex of the triangle fan
	glVertex2f(dx, dy);
}
...
for (angle = 0.0f; angle <= 360; angle += 5)
{
	...
	// Increment flag to change color next time
	flag++;
