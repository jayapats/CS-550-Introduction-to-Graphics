...
const char* WINDOWTITLE = { "OpenGL / GLUT Sample -- Swetha Jayapathy" };
...
GLuint	HeliList;			    // Helicopter body
GLuint  BladeList;			    //Front Blade
GLuint	TailBladeList;			//Tail Blade
GLuint	BoxList;			    // Helicopter body
...
float	blAngle=0.f;			//Angle of the blade
int		viewFrom;
...
Display()
{
	...
	if (viewFrom == 0) {
		...
		gluLookAt(-0.4, 1.8, -4.9, 0., 3., -13., 0., 1., 0.);
	}
	//Outside View
	else if (viewFrom == 1) {
	...
	glRotatef(blAngle * 3, 1., 0., 0.);
	glRotatef(90., 0., 1., 0.);
	glScalef(1.5, 1.5, 1.5);
	glCallList(BladeList);
	glPopMatrix();

	//object above helicopter:
	glPushMatrix();
	glCallList(BoxList);
	glPopMatrix();
	...
	// text that just rotates on top of the scene:

	glDisable(GL_DEPTH_TEST);
	glColor3f(1., 1., 1.);
	DoRasterString(0., 1., 0., (char*)"On boarding");
	...
	glLoadIdentity();
	glColor3f(1., 1., 1.);
	DoRasterString(5., 5., 0., (char*)"Swetha Jayapathy");
...
//	width BLADE_WIDTH centered at (0.,0.,0.) in the XY plane
	...
	glTranslatef(2.f, 6.f, -15.f);  // Move left and into the screen
	//glVertex3f(2., 6., -15);
