...
InitMenus( )
{
	...
	glutAddMenuEntry("Add Texture", 1);
...
InitGraphics( )
{
	...
	//unsigned char* texture = BmpToTexture("sat.bmp", &width, &height);
	...
	for (int ilat = 0; ilat < NumLats; ilat++)
	{
		...
		for (int ilng = 0; ilng < NumLngs; ilng++)
		{
			...
			if (Distort)
			{
				...
				//p->t = (lat + M_PI / 2.) / M_PI + sin(lng+countInc)*0.30;
				//p->t = (lat + M_PI / 2.) / M_PI;
				//vertically distort
				p->t = (lat + M_PI / 2.) / M_PI + (cos(lng + M_PI + countInc) /2.)* 0.20;
			}
			else
			{
				p->s = (lng + M_PI) / (2. * M_PI);
