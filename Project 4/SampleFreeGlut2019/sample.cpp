...
const char* WINDOWTITLE = { "OpenGL / GLUT Sample -- Project#4 Swetha" };
...
//To switch the light sources on and off
bool Light0 = 1; //Press 0 on Keyboard for White point light
bool Light1 = 1; //Press 1 on Keyboard for revolving yellow light
bool Light2 = 1; //Press 1 on Keyboard for Red Point light
bool Freeze = 0;
...
Display()
{
	...
	gluLookAt(-2.8, 3.3, 6., 0., 0., 0., 0., 1., 0.);
	...
	//Light 0	
	//void SetPointLight(int ilight, float x, float y, float z, float r, float g, float b)
	SetPointLight(GL_LIGHT0, -5., 3., 0., 1., 1., 1.);
	glPushMatrix();
	...
	SetSpotLight(GL_LIGHT1, 8. * -cos(1.5 * M_PI * Time), 8. * -sin(1.5 * M_PI * Time), 0., cos(1.5 * M_PI * Time), 0., sin(1.5 * M_PI * Time), 0.5, 1., 0.);
	glPushMatrix();
	glDisable(GL_LIGHTING);
	glColor3f(1., 1., 0.);
	glTranslatef(8. * -cos(1.5 * M_PI * Time), 0., 8. * -sin(1.5 * M_PI * Time));
	...
	//Light 2
	//void SetPointLight(int ilight, float x, float y, float z, float r, float g, float b)
	SetPointLight(GL_LIGHT2, 13., 2., 0.5, 1., 0., 0.);
	glPushMatrix();
	glDisable(GL_LIGHTING);
	glColor3f(1., 0., 0.);
	glTranslatef(13., 2., -4.);
	...
	//Flat Shaded object - torus	
	glPushMatrix();
	glShadeModel(GL_FLAT);
	glTranslatef(-5., 2., -3.);
	//void SetMaterial(float r, float g, float b, float shininess)
	SetMaterial(0.9, 0.8, 0, 100);
	glColor3f(0.9, 0.8, 0.);
	//glutSolidSphere(0.75, 50, 50);
	glutSolidTorus(0.9,2,5, 30);
	glPopMatrix();
	...
	glShadeModel(GL_FLAT);
	glTranslatef(0., -6., 3.);
	SetMaterial(0.5, 0.8, 0.5, 100);
	glutSolidIcosahedron();
	glPopMatrix();

	//Rotating object with glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
	glEnable(GL_TEXTURE_2D);
	glTexEnvf(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_MODULATE);
	//glBindTexture(GL_TEXTURE_2D, PlanetList);

	glPushMatrix();
	glShadeModel(GL_SMOOTH);
	glTranslatef(cos(2 * M_PI * Time) * 6., 2., sin(2 * M_PI * Time) * 6.);
	glRotatef(360. * Time, 0., 2., 0.);
	SetMaterial(1.0, 1.0, 1.0, 0.5);
	glBindTexture(GL_TEXTURE_2D, PlanetList);
	MjbSphere(1.1, 100, 100);
	...
	glLoadIdentity();
	glColor3f(1., 1., 1.);
	DoRasterString(5., 5., 0., (char*)"Project 4 by Swetha");
