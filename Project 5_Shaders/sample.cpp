...
const char *WINDOWTITLE = { "OpenGL / GLUT Sample -- Project 5_Jayapats" };
...
Display( )
{
	...
	float  Ds=1; 
	float  Dt=1;  
	float  V0 = 0.2, V1 = 0.5, V2 = 1.2;
	float  Size = 1;
	float  ColorR = 0.5, ColorG = 1., ColorB = 1.0;
	float  uKa = 0.5, uKd = 0.5, uKs = 1.2;	// coefficients of each type of lighting
	float  uSpecularColorR = 0.9, uSpecularColorG = 1, uSpecularColorB = 0.9; //vec3  uSpecularColor;	// light color
	float  Shininess = 30;
	...
	Pattern->SetUniformVariable("uSpecularColor", uSpecularColorR, uSpecularColorG, uSpecularColorB);
	...
	Pattern->SetUniformVariable("verShaderOn", VerShaderOn);
	Pattern->SetUniformVariable("fragShaderOn", FragShaderOn);
	Pattern->SetUniformVariable("fragIdle", fragIdle);
...
Keyboard( unsigned char c, int x, int y )
{
	...
	switch( c )
	{
		...
		case 'b':
			...
			both = true;
			if (both) {
				FragShaderOn = !FragShaderOn;
				VerShaderOn = !VerShaderOn;
			}
			else
			{
				FragShaderOn = 0;
	...
	for (int ilat = 0; ilat < NumLats; ilat++)
	{
		...
		for (int ilng = 0; ilng < NumLngs; ilng++)
		{
			...
			if (Distort)
			{
				...
				p->s = (lng + M_PI) / (2. * M_PI) + sin(angle + (1 - (lat + M_PI / 2.) / M_PI) / 1);
				p->t = (lat + M_PI / 2.) / M_PI;
			}
			else
			{
