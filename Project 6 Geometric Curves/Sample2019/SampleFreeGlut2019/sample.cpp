...
const char *WINDOWTITLE = { "OpenGL / GLUT Sample -- Project 1_Jayapats" };
...
void DrawCurve(struct Curve* curve)
{
	...
	if (ControlCurve) {
		...
		for (int n = 0; n <= TOTPOINTS; n++)
		{
			float t = (float)n / (float)TOTPOINTS;
			float m = 1.f - t;
			float x = m * m * m * p0.x + 3.f * t * m * m * p1.x + 3.f * t * t * m * p2.x + t * t * t * p3.x;
			float y = m * m * m * p0.y + 3.f * t * m * m * p1.y + 3.f * t * t * m * p2.y + t * t * t * p3.y;
			float z = m * m * m * p0.z + 3.f * t * m * m * p1.z + 3.f * t * t * m * p2.z + t * t * t * p3.z;
...
Display( )
{
	...
	float btime = (sin(Time * 2 * M_PI) + 1) / 2.5;
	 // Draw the Butterfly	
	glPushMatrix();
	int num = 10;
	for (int c = 0; c < num; c++) {
		float angle = 360. * c / num;
		struct Curve curve;
	
		struct Point p0 = { 0., 0., 0. };
		struct Point p3 = { 0., 0., 0. };
		struct Point p1 = { 2+cos(angle)*btime , 1.*btime, 2+sin(angle) };
		struct Point p2 = { 2 + cos(angle / 3)*btime , 2.*btime , 2 + sin(angle * 3) };
	...
	for (int c = 0; c < num; c++) {
		float angle = 360. * c / num;
		struct Curve acurve;
		struct Point ap0 = { 0., 0., 0. };
		struct Point ap3 = { 0., 0., 0. };
		struct Point ap1 = { -(2 + cos(angle)*btime) , 1.*btime, 2 + sin(angle) };
		struct Point ap2 = { -(2 + cos(angle / 3)*btime) , (2. - btime)*btime , 2 + sin(angle * 3) };
	...
	for (int c = 0; c < num; c++) {
		...
		struct Point ap0 = { 0., 0., 0. };
		struct Point ap3 = { 0., 0., 0. };
		struct Point ap1 = { -2 + sin(angle) , -0.7, 2 + cos(angle) };
		struct Point ap2 = { -2 + sin(angle / 3) , -1.2 - btime , 2 + cos(angle * 3) };
		acurve.p0 = ap0;
		acurve.p1 = ap1;
		acurve.p2 = ap2;
		acurve.p3 = ap3;
	...
	glPushMatrix();
		...
		struct Point p2 = { -0.5, 1. ,2 };
		struct Point p3 = { -0.2, 0.7 , 2. };
		curve.p0 = p0;
		curve.p1 = p1;
		curve.p2 = p2;
		curve.p3 = p3;
	...
	glPushMatrix();
	struct Curve bcurve;
	struct Point cp0 = { 0., 0., 0. };
	struct Point cp1 = { 0 , 1., 2 };
	struct Point cp2 = { 0.5, 1. ,2 };
	struct Point cp3 = { 0.2, 0.7 , 2. };
	bcurve.p0 = cp0;
	bcurve.p1 = cp1;
	bcurve.p2 = cp2;
	bcurve.p3 = cp3;
	...
	glPushMatrix();
	struct Curve ncurve;
	struct Point np0 = { 0., -0.4, 0. };
	struct Point np1 = { 0.1 , 0.4, 0 };
	struct Point np2 = { -0.1, 0.4 ,0 };
	struct Point np3 = { 0, -0.4 , 0 };
	ncurve.p0 = np0;
	ncurve.p1 = np1;
	ncurve.p2 = np2;
	ncurve.p3 = np3;
